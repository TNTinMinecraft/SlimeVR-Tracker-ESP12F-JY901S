...
class JY901 
{
  public: 
    ...
    JY901 (unsigned char ucAddr = 0x50); 
	unsigned char StartIIC(unsigned char ucAddr = 0x50);
	...
	bool GetTapDetector();
	void Unlock();
	void Save(unsigned char save);
	void SetDirection(unsigned char dir);
	void SetALG(unsigned char alg);
	void SetCalsw(unsigned char cal);
